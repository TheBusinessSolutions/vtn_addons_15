...
class LotSerialExpiryReportWizard(models.TransientModel):
	...
	def onchange_lot_serial_wise(self):
		if self.selection_report == 'lot_wise':
			stock_production_lot_obj = self.env['stock.production.lot'].search([])
			self.lot_serial_ids = stock_production_lot_obj
			self.product_ids = False

	def action_expiry_report(self):
		product_list = []
		res_config_settings_obj=self.env['res.config.settings'].search([])
		values_get = res_config_settings_obj.default_get('')
		product_expiry_boolean = values_get.get("module_product_expiry")

		if product_expiry_boolean == True:
			...
			if self.selection_report == 'product_wise':
				if self.expire_within > 0:
					for record in self.product_ids:
						stock_production_lot_obj = self.env['stock.production.lot'].search([('product_id','=', record.id)])
			...
			if self.selection_report == 'lot_wise':
				if self.expire_within > 0:
					for rec in self.lot_serial_ids:
						if rec.expiration_date:
							stock_production_lot_obj = self.env['stock.production.lot'].browse([(rec)]).id
